...
void CMD5Encrypt::EncryptData(LPCTSTR pszSrcData, TCHAR szMD5Result[33])
{
	...
	CMD5 MD5Encrypt;
	UCHAR szResult[16];
	MD5Encrypt.MD5Update((unsigned char *) W2A(pszSrcData),lstrlen(pszSrcData)*sizeof(char));
	MD5Encrypt.MD5Final(szResult);

	//输出结果
	szMD5Result[0]=0;
	for (int i=0;i<16;i++) 
		_sntprintf(&szMD5Result[i*2], CountArray(szMD5Result), TEXT("%02x"), szResult[i]);
...
WORD CXOREncrypt::CrevasseData(LPCTSTR pszEncrypData, LPTSTR pszSrcData, WORD wSize)
{
	...
	for (int i=0;i<wSoureLength;i++)
	{
		...
		szTempBuffer[2]=0;
		UINT uKey=strtol((const char*)szKeyBuffer,&pEnd,16);
