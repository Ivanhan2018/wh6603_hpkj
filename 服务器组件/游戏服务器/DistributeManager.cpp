...
WORD  CDistributeManager::PerformDistribute(CDistributeInfoArray & DistributeInfoArray,WORD wNeedCount)
{
	...
	while(pMoveNode!=NULL)
	{
		...
		//机器过滤  //判断是否全部是机器人
		if(bFirstSuccess==TRUE && DistributeInfoArray.GetCount()==wNeedCount-1 && FilterRuleIsAllAndroid(DistributeInfoArray,pMoveNode->DistributeInfo.pIServerUserItem))
		{
			bFirstSuccess=FALSE;
		}

		//控制号真人过滤  //判断是否存在真人
		if(bFirstSuccess==TRUE &&  FilterRuleIsAllReal(DistributeInfoArray,pMoveNode->DistributeInfo.pIServerUserItem))
		{
			bFirstSuccess=FALSE;
		}

		//同IP过滤
		if(bFirstSuccess==TRUE && (m_cbDistributeRule&DISTRIBUTE_SAME_ADDRESS)==0 && FilterRuleExitsIPAddr(DistributeInfoArray,pMoveNode->DistributeInfo.dwClientAddr)==TRUE)
		{
			bFirstSuccess=FALSE;
...
BOOL  CDistributeManager::FilterRuleExitsIPAddr(const CDistributeInfoArray & DistributeInfoArray,DWORD dwClientAddr)
{
	...
	for(INT_PTR nIndex=0;nIndex<DistributeInfoArray.GetCount();nIndex++)
	{
		//EWIN网络
		//跳过机器人
		//if(DistributeInfoArray[nIndex].pIServerUserItem->IsAndroidUser()==true) return FALSE;
		if(DistributeInfoArray[nIndex].dwClientAddr==dwClientAddr) return TRUE;
	}
...
//控制号真人控制
BOOL CDistributeManager::FilterRuleIsAllReal(const CDistributeInfoArray & DistributeInfoArray,IServerUserItem  * const pIServerUserItem)
{
	///这里要判断所有用户
	//参数校验
	if(pIServerUserItem==NULL || DistributeInfoArray.GetCount()==0) return FALSE;
	...
	WORD wRealCount=0;
	//当前分配用户是否控制号
	bool IsAdmin=CUserRight::IsGameCheatUser(pIServerUserItem->GetUserRight());
	//一个真人
	if(pIServerUserItem->IsAndroidUser()==false)  wRealCount++;
	//统计其他真人
	for(INT_PTR nIndex=0;nIndex<DistributeInfoArray.GetCount();nIndex++)
	{
		//不是机器人
		if(DistributeInfoArray[nIndex].pIServerUserItem->IsAndroidUser()==false) wRealCount++;
		//存在控制号
		if(CUserRight::IsGameCheatUser(DistributeInfoArray[nIndex].pIServerUserItem->GetUserRight())&&IsAdmin==false) IsAdmin=true;
	}
							 //控制号存在
	return (wRealCount<2) && IsAdmin;
}
