...
#define USER_VIEW_CY				153									//用户视图	//G更高
...
//	m_pTrumpetItem=NULL;
	...
	EncircleChat.pszImageGR=MAKEINTRESOURCE(IDB_CHAT_GR);
	...
	EncircleUserList.pszImageGR=MAKEINTRESOURCE(IDB_USER_LIST_GR);
...
//david,2015.5.8			m_UserListControl.SetServerRule(pServerAttribute->dwServerRule);
	...
	{
		CString strNick = pIClientUserItem->GetNickName();
		strNick = strNick.Left(4);
		strNick+=_T("****");
		m_ChatMessage.InsertUserEnter(strNick);
	}
...
VOID CGameFrameControl::OnEventUserLeave(IClientUserItem * pIClientUserItem, bool bLookonUser)
{
	...
	if (pGlobalUnits->m_bNotifyUserInOut==true)
	{
		...
		m_ChatMessage.InsertUserLeave(strNick);
	}
...
VOID CGameFrameControl::OnEventUserScore(IClientUserItem * pIClientUserItem, bool bLookonUser)
{
	...
	{
		OutputDebugString(L"USERSCORE 更新分数");
		//m_GameFrameUserView.SetUserViewInfo(pIClientUserItem);
...
VOID CGameFrameControl::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_BT_CHAT_BUTTON_5, m_btChatButton5);
...
//	ZeroMemory(&PropertyResource,sizeof(PropertyResource));
	...
	m_ChatEdit.SetEnableColor(RGB(0,0,0),RGB(86,73,71),RGB(86,73,71));

	//聊天控件
	m_ChatControl.SetChatControlSink(this);
	m_ChatMessage.SetExpressionManager(CExpressionManager::GetInstance(),RGB(86,73,71));
	...
	m_btChatButton5.SetButtonImage(IDB_BT_CHAT_BUTTON_5,TEXT("BT_CHAT_BUTTON_5"),hInstance,false,false);
	...
	m_btChatButton5.ShowWindow(SW_HIDE);
	//查询接口
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	CSkinButton * ChatButtonArray[]={&m_btChatButton1,&m_btChatButton2,&m_btChatButton3,&m_btChatButton4,&m_btChatButton5};

	//聊天控件
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-33,rcChatArea.bottom-22,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-53,rcChatArea.bottom-22,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+12,rcChatArea.bottom-19,rcChatArea.Width()-60,12,uFlags);
	...
	for (INT i=0;i<CountArray(ChatButtonArray);i++)
	{
		...
		UINT uTargetFlag=SWP_SHOWWINDOW;
		...
		ChatButtonArray[i]->ShowWindow(SW_HIDE);
	}
...
VOID CGameFrameControl::RectifyControl(CSkinRenderManager * pSkinRenderManager)
{
	...
	ColorUserList.crNormalTX=RGB(233,157,159);//COLOR_NORMAL_TX;
	ColorUserList.crNormalBK=RGB(86,73,71);//COLOR_NORMAL_BK;

	//颜色定义
	ColorUserList.crMyselfTX=COLOR_MYSELF_TX;
	ColorUserList.crMyselfBK=COLOR_MYSELF_BK;
	ColorUserList.crMasterTX=RGB(233,157,159);
	ColorUserList.crMasterBK=RGB(86,73,71);
