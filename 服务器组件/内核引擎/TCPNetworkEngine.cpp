...
WORD CTCPNetworkItem::EncryptBuffer(BYTE pcbDataBuffer[], WORD wDataSize, WORD wBufferSize)
{
	...
	for (INT_PTR i = 0; i < wEncrypCount; i++)
	{
    *pdwXor++ ^= dwXorKey;
...
{
	CString strLog;
	strLog.Format(L"ASSERTSEND wMainCmdID:%d,wSubCmdID:%d,wDataSize:%d",wMainCmdID,wSubCmdID,wDataSize);
...
bool CTCPNetworkEngine::WebAttestation()
{
	...
	try
	{
		...
		//http://www.083588.com/Attestation.html?Version=66&ClientID=%ld&ClientName=%s&Validate=%s
		_sntprintf(szAttestation,CountArray(szAttestation),TEXT(""),
		...
		if ((uReadCount>=2)&&(szResult[0]==TEXT('0'))&&(szResult[1]==TEXT(';')))
		{
			...
			//提示信息
			g_TraceServiceManager.TraceString(TEXT("您的服务器组件没有得到合法授权"),TraceLevel_Exception);
...
CTCPNetworkItem * CTCPNetworkEngine::GetNetworkItem(WORD wIndex)
{
	...
	if(m_NetworkItemStorage.IsEmpty())
		return NULL;
	//效验索引
