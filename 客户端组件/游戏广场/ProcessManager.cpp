...
//发送数据
bool CProcessManager::SendProcessData(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize)
{
	//状态效验
	ASSERT(m_ProcessControlModule.GetInterface()!=NULL);
	if (m_ProcessControlModule.GetInterface()==NULL) return false;
...
//执行命令
bool CProcessManager::SendProcessCommand(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize)
{
	//状态效验
	ASSERT(m_ProcessControlModule.GetInterface()!=NULL);
	if (m_ProcessControlModule.GetInterface()==NULL) return false;
...
//房间信息
bool CProcessManager::SendServerInfo(IClientUserItem * pIMyselfUserItem, DWORD dwUserRight, DWORD dwMasterRight, DWORD dwServerRule, WORD wServerType,BOOL bAllowDistribute,const tagGameServer & GameServer)
{
	//房间信息
	...
	ServerInfoData.bAllowDistribute=bAllowDistribute;
...
//用户信息
bool CProcessManager::SendUserSpreadInfo(DWORD dwUserID, tagUserSpreadInfo & UserSpreadInfo)
{
	//用户状态
	IPC_GF_UserSpreadInfo UserSpreadInfoData;
	ZeroMemory(&UserSpreadInfoData,sizeof(UserSpreadInfoData));

	//构造信息
	UserSpreadInfoData.dwUserID=dwUserID;
	UserSpreadInfoData.UserSpreadInfo=UserSpreadInfo;

	//发送数据
	SendProcessData(IPC_CMD_GF_USER_INFO,IPC_SUB_GF_USER_SPREADINFO,&UserSpreadInfoData,sizeof(UserSpreadInfoData));
