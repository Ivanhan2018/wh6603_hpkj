...
#define TIME_REPLACE_CARD			1									//补花时间
#else
...
#define IDI_REPLACE_CARD			(IDI_ANDROID_ITEM_SINK+3)			//补花时间
...
bool  CAndroidUserItemSink::OnEventTimer(UINT nTimerID)
{
	...
	case IDI_START_GAME:	//开始游戏
		{
			////////m_pIAndroidUserItem->KillGameClock(IDI_START_GAME);
	...
	case IDI_REPLACE_CARD:		//补花定时器
		{
			m_pIAndroidUserItem->KillGameTimer( IDI_REPLACE_CARD );

			WORD wMeChairId = m_pIAndroidUserItem->GetChairID();
			//效验
			ASSERT(wMeChairId == m_wReplaceUser );

			m_wReplaceUser = INVALID_CHAIR;
			
			//搜索花牌
			BYTE cbReplaceCard = 0;
			for( BYTE i = MAX_INDEX-MAX_HUA_CARD; i < MAX_INDEX; i++ )
			{
				if( m_cbCardIndex[wMeChairId][i] == 0 ) continue;

				cbReplaceCard = m_GameLogic.SwitchToCardData(i);
				if( !m_GameLogic.RemoveCard(m_cbCardIndex[wMeChairId],cbReplaceCard) )
				{
					ASSERT( FALSE );
					return true;
				}
			...
			ASSERT(cbReplaceCard!=0);
			//发送数据
			CMD_C_ReplaceCard ReplaceCard;
			ReplaceCard.cbCardData=cbReplaceCard;
			m_pIAndroidUserItem->SendSocketData(SUB_C_REPLACE_CARD,&ReplaceCard,sizeof(ReplaceCard));
...
bool CAndroidUserItemSink::OnSubGameStart(void * pData, WORD wDataSize)
{
	...
	{
		UINT nElapse = rand()%TIME_REPLACE_CARD+TIME_LESS+5;
		m_pIAndroidUserItem->SetGameTimer(IDI_REPLACE_CARD,nElapse);
	}
...
//操作通知
bool CAndroidUserItemSink::OnSubOperateNotify( void *pBuffer,WORD wDataSize )
{
	//效验数据
...
//操作结果通知
bool CAndroidUserItemSink::OnSubOperateResult( void *pBuffer,WORD wDataSize )
{
	//效验消息
...
//用户出牌
bool CAndroidUserItemSink::OnSubOutCard( void *pBuffer,WORD wDataSize )
{
	//效验消息
...
//用户发牌
bool CAndroidUserItemSink::OnSubSendCard( void *pBuffer,WORD wDataSize )
{
	//效验数据
	...
	{
		m_cbCardIndex[pSendCard->wSendCardUser][m_GameLogic.SwitchToCardIndex(pSendCard->cbCardData)]++;	
		//扣除扑克
		m_cbLeftCardCount--;
	}
	...
	//补牌操作
	if ( pSendCard->wReplaceUser==wMeChairID )
	{
		//计算时间
		UINT nElapse=rand()%TIME_REPLACE_CARD+TIME_LESS;
		m_pIAndroidUserItem->SetGameTimer(IDI_REPLACE_CARD,nElapse);
	}
...
//补牌消息
bool CAndroidUserItemSink::OnSubReplaceCard(void * pBuffer, WORD wDataSize)
{
	//效验参数
	...
	//其他用户
	if (pReplaceCard->wReplaceUser!=wMeChairID)
	{
		//删除扑克
		if( !m_GameLogic.RemoveCard(m_cbCardIndex[pReplaceCard->wReplaceUser],pReplaceCard->cbReplaceCard) )
		{
			ASSERT( FALSE );
			return false;
		}
