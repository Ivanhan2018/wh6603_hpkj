...
bool  CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	//TCHAR str[222];
	//sprintf(str,"%s，%s",m_hInst?"有权限":"没权限",m_pServerContro?"有权限":"没权限");
	//AfxMessageBox(str);
	...
	case GER_NORMAL:		//常规结束	
		{
			...
			//TCHAR szTemp1[255]=TEXT("");
			//swprintf(szTemp1, sizeof(szTemp1),TEXT("游戏结束,当前库存：%I64d"),m_lStorageStart);
			...
			swprintf(szTemp, sizeof(szTemp),TEXT("游戏结束,当前库存：%I64d"),m_lStorageStart);
	...
	case SUB_C_AMDIN_COMMAND:
		{
			...
			//TCHAR str[222];
			//sprintf(str,"1%s，%s",m_hInst?"有权限":"没权限",m_pServerContro?"有权限":"没权限");
			//AfxMessageBox(str);
			if ( m_pServerContro == NULL)
				return false;

			//sprintf(str,"2%s，%s",m_hInst?"有权限":"没权限",m_pServerContro?"有权限":"没权限");
			//AfxMessageBox(str);
...
bool CTableFrameSink::OnUserPlaceJetton(WORD wChairID, BYTE cbJettonArea, LONGLONG lJettonScore)
{
	...
	if (bPlaceJettonSuccess)
	{
		...
		CString strArea[4]={L"东",L"南",L"西",L"北"};
		for(int i=0; i<MAX_CHAIR; i++)
		{
			...
			myprintf(strMessage, sizeof(strMessage), _T("%s:[%s]下注%I64d"), pIServerUserItem->GetUserInfo()->szNickName,strArea[cbJettonArea-1],lJettonScore);
			...
			if (wBankerTime+m_wAddTime<=m_wBankerTime || lBankerScore<m_lApplyBankerCondition)
			{
				...
				if(wBankerTime <= m_wBankerTime && m_bExchangeBanker && lBankerScore >= m_lApplyBankerCondition)
				{
					...
					if ( bScoreMAX || (lBankerScore > m_lExtraBankerScore && m_lExtraBankerScore != 0l) )
					{
						...
						TCHAR szTipMsg[128] = {};
						if (bType == 1 )
							_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("[ %s ]的分数超过其他申请上庄玩家，获得%d次额外坐庄次数!"),strNick,m_wAddTime);
						else if (bType == 2)
							_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("[ %s ]的分数超过[%I64d]，获得%d次额外坐庄次数!"),strNick,m_lExtraBankerScore,m_wAddTime);
						else if (bType == 3)
							_sntprintf(szTipMsg,CountArray(szTipMsg),TEXT("[ %s ]的分数超过[%I64d]且超过其他申请玩家，获得%d次额外坐庄次数!"),strNick,m_lExtraBankerScore,m_wAddTime);
						else
							bType = 0;
...
//					m_nTotalWin -=  ( pUserScore[wAreaIndex][wChairID] * cbMultiple[wAreaIndex] ) ;
		...
		/*	myprintf(szBuffer,CountArray(szBuffer),TEXT(" 本局分数 %ld 返回分数 %d 用户名字%s 用户分数%d\r\n"),m_lUserWinScore[wChairID],m_lUserRevenue[wChairID],
	...
	if (m_wCurrentBanker!=INVALID_CHAIR)
	{
		...
		if(!pIServerUserItem->IsAndroidUser())
		{
			if(lBankerWinScore >= 0)
				m_nTotalWin += lBankerWinScore;				//Banker Win,  增长分子；
			else
				m_nTotalPay -= lBankerWinScore;				//Banker Lose, 增长分母；
		}
