...
CServerUserItem::CServerUserItem()
{
	...
	m_cbPlazaType=PLAZA_TYPE_NORMAL;
...
//写入积分
bool CServerUserItem::WriteUserScore(SCORE lScore, SCORE lRevenue, SCORE lUserMedal,BYTE cbScoreType, DWORD dwPlayTimeCount)
{
	//效验状态
...
//地址变更
bool CServerUserItem::SetIpAddrDescribe(LPCTSTR pszAddrDescibe)
{
	//设置变量
	CopyMemory(m_UserInfo.szAddrDescribe,pszAddrDescibe,sizeof(m_UserInfo.szAddrDescribe));
	
	//发送状态
	ASSERT(m_pIServerUserItemSink!=NULL);
	if (m_pIServerUserItemSink!=NULL) m_pIServerUserItemSink->OnEventUserItemSpreadInfo(this);
...
bool CServerUserItem::FrozenedUserScore(SCORE lScore)
{
	...
	//效验积分
	ASSERT((SCORE_EQUAL(lScore,0L)==false)&&SCORE_GREATER_EQUAL(m_UserInfo.lScore+m_lTrusteeScore,lScore));
	if (SCORE_LESS(lScore,0L)|| SCORE_LESS(m_UserInfo.lScore+m_lTrusteeScore,lScore)) return false;

	//设置变量
	m_lFrozenedScore+=lScore;

	//设置用户
	if (SCORE_LESS(m_lTrusteeScore,lScore))
	{
		//积分锁定
...
bool CServerUserItem::UnFrozenedUserScore(SCORE lScore)
{
	...
	//效验积分
	ASSERT(SCORE_GREATER_EQUAL(lScore,0L)&&SCORE_GREATER_EQUAL(m_lFrozenedScore,lScore));
	if (SCORE_LESS(lScore,0L)|| SCORE_LESS(m_lFrozenedScore,lScore)) return false;
...
{
	//效验参数
	//ASSERT(SCORE_EQUAL(lScore+lInsure+lRevenue,0L)&&SCORE_GREATER_EQUAL(lRevenue,0L));
	//if ((SCORE_EQUAL(lScore+lInsure+lRevenue,0L)==false)||SCORE_LESS(lRevenue,0L)) return false;

	//效验状态
	//ASSERT((m_UserInfo.dwUserID!=0L)&&SCORE_GREATER_EQUAL(m_UserInfo.lScore+m_lTrusteeScore+lScore,0L));
	//if ((m_UserInfo.dwUserID==0L)||SCORE_LESS(m_UserInfo.lScore+m_lTrusteeScore+lScore,0L)) return false;
...
bool CServerUserManager::InsertUserItem(IServerUserItem * * pIServerUserResult, tagUserInfo & UserInfo, tagUserInfoPlus & UserInfoPlus)
{
	...
	pServerUserItem->m_cbPlazaType=UserInfoPlus.cbPlazaType;
	...
	//托管调整
	if (SCORE_GREATER(UserInfoPlus.lRestrictScore,0L)&& SCORE_GREATER(UserInfo.lScore,UserInfoPlus.lRestrictScore))
	{
