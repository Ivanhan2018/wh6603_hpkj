...
CGameClientEngine::CGameClientEngine()
{
	...
	//加注信息
	m_lMaxScore=SCORE_D_ZERO;
	m_lMaxCellScore=SCORE_D_ZERO;
	m_lCellScore=SCORE_D_ZERO;
	m_lCurrentTimes=1.000;
	m_lUserMaxScore=SCORE_D_ZERO;
...
bool CGameClientEngine::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	//设置变量
	SCORE lTableScore=SCORE_D_ZERO;
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		m_wLostUserID[i]=INVALID_CHAIR;
...
bool CGameClientEngine::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	SCORE lTax = m_lCellScore*(pGameEnd->lGameTax/1000.);
	WORD wWinner=INVALID_CHAIR;
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		//设置信息
		if (pGameEnd->lGameScore[i]>0.001||pGameEnd->lGameScore[i]<-0.001)
		{
			if(pGameEnd->lGameScore[i]>0.001) 
			{
				wWinner = i;
			}
	...
	//数字滚动动画
	SCORE GameEndSocre[GAME_PLAYER];
	...
	if (IsLookonMode()==false)
	{
		if (pGameEnd->lGameScore[GetMeChairID()]>0.001) PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_WIN"));
...
void CGameClientEngine::UpdataControl()
{
	...
	//判断开牌
	SCORE lTemp=(m_bMingZhu[wMeChairID])?6:5;
	if((m_lUserMaxScore-m_lTableScore[wMeChairID]) >= (m_lMaxCellScore*lTemp))
	{
		...
		//跟注按钮
		m_GameClientView.m_btFollow.EnableWindow(((abs(m_lTableScore[j]-m_lCellScore))<0.001)?FALSE:TRUE);

		//加注按钮
		m_GameClientView.m_btAddScore.EnableWindow(((m_lCurrentTimes*m_lCellScore) < m_lMaxCellScore)?TRUE:FALSE);

		//比牌按钮
		if(wMeChairID==m_wBankerUser || m_lTableScore[wMeChairID]>=2*m_lCellScore-0.001)
		{
...
LRESULT	CGameClientEngine::OnAddScore(WPARAM wParam, LPARAM lParam)
{
	...
	//处理控件
	m_GameClientView.m_JettonView.SetSocreInfo(m_lMaxCellScore,m_lCellScore*m_lCurrentTimes);
...
LRESULT CGameClientEngine::OnConfirmScore(WPARAM wParam, LPARAM lParam)
{
	...
	//效验数据
	ASSERT(wTemp==1 || m_GameClientView.m_JettonView.GetCheckScore() >= ((m_lCellScore*m_lCurrentTimes)-0.001)
	...
	//获取筹码
	SCORE lCurrentScore=(wTemp==0)?(m_GameClientView.m_JettonView.GetCheckScore()):(m_lCellScore*m_lCurrentTimes);
	WORD wMeChairID=GetMeChairID();
...
LRESULT CGameClientEngine::OnFlashCardFinish(WPARAM wParam, LPARAM lParam)
{
	...
	//if(pClientUserItem!=NULL && pClientUserItem->GetUserStatus()!=GAME_STATUS_PLAY && pClientUserItem->GetUserStatus()!=US_OFFLINE&&m_cbPlayStatus[GetMeChairID()]==FALSE && GetMeChairID()!=m_wLostUser)
	//{
...
LRESULT CGameClientEngine::OnAdminQueryUser(WPARAM wParam,LPARAM lParam)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		m_wGameID[i] = pClientUserItem->GetGameID();
	}
