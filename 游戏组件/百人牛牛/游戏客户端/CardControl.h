...
#define DEF_SHOOT_DISTANCE			40									//默认间距

//间距定义
#define DEF_X_DISTANCE_SMALL		17									//默认间距
#define DEF_Y_DISTANCE_SMALL		17									//默认间距
#define DEF_SHOOT_DISTANCE_SMALL	40									//默认间距
...
public:
	...
	///得到帧
	int GetFrame(int nIndex){return m_nFrameIndex[nIndex];};
	///
	void AddFrame(int nIndex){m_nFrameIndex[nIndex] ++; if(m_nFrameIndex[nIndex] >= 8 ) m_nFrameIndex[nIndex] = 8;};
	///设置帧
	void SetFrame(int nFrame,int nIndex){if(m_nFrameIndex[nIndex] == 8) 
											m_nFrameIndex[nIndex] = 8;
										else
											m_nFrameIndex[nIndex] = nFrame;};
	void ResetFrame(){ZeroMemory(&m_nFrameIndex,sizeof(int) * 5);};
	///设置源点
	void SetPtSrc(CPoint ptSrc){m_ptSrc = ptSrc;};
	int GetCardNum(){return (int)m_CardItemArray.GetCount();};
	//扑克控制
public:
	//删除扑克
