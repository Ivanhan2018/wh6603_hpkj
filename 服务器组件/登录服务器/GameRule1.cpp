...
bool CChqSSCRule::IsCanCancel(CString qihao)
{
	...
	CString strLog;
	strLog.Format(L"CANCANCEL CHQSSC qihao:%s,GetNext:%s,kjshj:%d",qihao,GetNextExpect(),GetKjShjDiff());
	OutputDebugString(strLog);
...
CString CXJSSCRule::GetNextExpect(int nDelta)
{
	...
	if (((t.GetHour() == 10 && t.GetMinute()>=10)||(t.GetHour()>10)) && t.GetHour() <= 24)
	{
		...
		if (tSpan.GetTotalSeconds() % timespan_fd_shj >= 540)
		{
...
//  	return rQh;
	int nQiHao =0;
	CTime tm= CTime::GetCurrentTime();//=CTime::GetCurrentTime();
	//theApp.GetTime(tm);
	tm+=m_timeSpan;
...
CTime CGD11X5Rule::GetNextKjShj()
{
	...
	CString rQh;
	if (((t.GetHour() == 9 && t.GetMinute()>=10)||t.GetHour() > 9) && (t.GetHour() < 23 ||(t.GetHour() == 23&& t.GetMinute()<1)) ){
		CTime starttime(t.GetYear(),t.GetMonth(),t.GetDay(),8,59,30);
		CTimeSpan tSpan = t-starttime;
		int nTotalSeconds = tSpan.GetTotalSeconds();
		int nQihao = 1;
		...
		CString strLog;
		strLog.Format(L"KAIJIANG %d:%d:%d,   %d",starttime.GetHour(),starttime.GetMinute(),starttime.GetSecond(),nTotalSeconds);
		OutputDebugString(strLog);
		return starttime;
	}
	else {
...
CString CBJPK10Rule::GetNextExpect(int nDelta)
{
	...
	{
		CTime daystart(tm.GetYear(),tm.GetMonth(),tm.GetDay(),9,7,35);
...
CTime CBJPK10Rule::GetNextKjShj()
{
	...
	if ( (tm.GetHour()>9)&&(tm.GetHour()<23)  || (tm.GetHour()==23)&&(tm.GetMinute()<57)||(tm.GetHour()==9)&&(tm.GetMinute()>=7))
	{
		...
		CString strLog;
		strLog.Format(L"BJPK10 %d:%d:%d      %d",NextKj.GetHour(),NextKj.GetMinute(),NextKj.GetSecond(),tCha.GetTotalSeconds()%300);
		OutputDebugString(strLog);
...
CString CHLJ11X5Rule::GetNextExpect(int nDelta)
{
	...
	if (((t.GetHour() == 8 && t.GetMinute()>=5)||t.GetHour()>8) && (t.GetHour() < 22 || (t.GetHour() == 22 && t.GetMinute()<5))){
		...
		int qishu = 1;
		CTime starttime(t.GetYear(),t.GetMonth(),t.GetDay(),7,53,0);
		CTimeSpan tSpan = t - starttime;
