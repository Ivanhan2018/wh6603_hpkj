...
public:
	...
	//发送数据
	bool SendProcessData(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize);
	//执行命令
	bool SendProcessCommand(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize);
...
public:
	...
	//房间信息
	bool SendServerInfo(IClientUserItem * pIMyselfUserItem, DWORD dwUserRight, DWORD dwMasterRight, DWORD dwServerRule, WORD wServerType,
		BOOL bAllowDistribute,const tagGameServer & GameServer);
...
public:
	...
	//用户信息
	bool SendUserSpreadInfo(DWORD dwUserID, tagUserSpreadInfo & UserSpreadInfo);
