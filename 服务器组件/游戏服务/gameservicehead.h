...
//////////////////////////////////////////////////////////////////////////////////
//常量定义
#define MAX_PT_MARK                 4                                   //标识数目<by hxh>
...
//开始模式
#define START_MODE_ALL_READY		0x01								//所有准备
#define START_MODE_FULL_READY		0x02								//满人开始
#define START_MODE_PAIR_READY		0x03								//配对开始
#define START_MODE_TIME_CONTROL		0x10								//时间控制
#define START_MODE_MASTER_CONTROL	0x20								//管理控制
...
struct tagGameServiceAttrib
{
	...
	//EWIN网络密码房间
	WORD							wServerKind;							//房间标识 
	TCHAR							szServerPassWork[LEN_SERVER];			//房间密码

	//EWIN网络虚拟人数
	WORD							wVirtualUserCount;						//虚拟人数
...
struct tagGameServiceOption
{
	...
	WORD                            wMinGameUser;                       //最少人数
	WORD                            wMaxGameUser;                       //最大人数 
...
interface IGameDataBaseEngineSink : public IUnknownEx
{
	...
	//数据处理
	virtual bool OnGameDataBaseRequest(DWORD dwUserID, WORD wTableID, WORD wChairID){return true;}
};
...
public:
	...
	//广场类型
	virtual BYTE  GetPlazeType()=NULL;
...
public:
	//地址变更
	virtual bool SetIpAddrDescribe(LPCTSTR pszAddrDescibe)=NULL;
...
public:
	...
	//写入积分
    virtual bool WriteUserScore(SCORE lScore, SCORE lRevenue, SCORE lUserMedal,BYTE cbScoreType, DWORD dwPlayTimeCount)=NULL;
	////修改权限
...
interface IServerUserItemSink : public IUnknownEx
{
	...
	//用户信息
	virtual bool OnEventUserItemSpreadInfo(IServerUserItem * pIServerUserItem)=NULL;
#else
	//用户积分
...
public:
	...
	//结束游戏
	virtual bool ConcludeGame(BYTE cbGameStatus,LPCTSTR szDrawInfo,WORD cbMcYes)=NULL;
...
public:
	//插入分配
	virtual bool InsertDistribute(IServerUserItem * pIServerUserItem,bool bCheckPlazaType=true)=NULL;
...
public:
	...
	////系统消息<by hxh>
	//virtual bool SendSystemNotice(LPCTSTR lpszMessage)=NULL;
