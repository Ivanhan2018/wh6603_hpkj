...
VOID CSkinComboBoxList::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	//计算位置
	CRect rcString;
	rcString.SetRect(4,3,rcItem.Width()-8,rcItem.Height());

	//颜色定义
	COLORREF crTextColor=((lpDrawItemStruct->itemState&ODS_SELECTED)!=0)?RGB(255,255,255):RGB(0,0,0);
	COLORREF crBackColor=((lpDrawItemStruct->itemState&ODS_SELECTED)!=0)?RGB(10,36,106):RGB(73,62,61);
...
VOID CSkinComboBoxList::OnNcPaint()
{
	...
	COLORREF crBoradFrame=RGB(73,62,61);//CSkinComboBox::m_SkinAttribute.m_crEnableBorad;
...
CSkinComboBox::CSkinComboBox()
{
	...
	m_uResourceID=0;
	m_hResInstance=NULL;
	m_crBackColor=m_SkinAttribute.m_crEnableBack;
	return;
}
...
VOID CSkinComboBox::DrawControlView(CDC * pDC)
{
	...
	if ((GetStyle()&0x000F)==CBS_DROPDOWNLIST)
	{
		...
		rcText.top+=2;
		rcText.bottom+=2;
		pDC->DrawText(strString,&rcText,DT_CENTER|DT_SINGLELINE);
	}
// 	else
// 	{
// 		//获取字符
...
// 		GetWindowText(strString);
// 		CString strLog;
// 		strLog.Format(L"COMBOBOX string:%s",strString);
...
// 	}
	...
	{
		COLORREF crEnableBorad=m_crBackColor;//m_SkinAttribute.m_crEnableBack;
		pDC->Draw3dRect(0,0,rcClient.Width(),rcClient.Height(),crEnableBorad,crEnableBorad);
	}
...
VOID CSkinComboBox::SetBackColor(COLORREF crBackColor)
{
...
VOID CSkinComboBox::RectifyControl(INT nWidth, INT nHeight)
{
	...
	if (m_SkinComboBoxEdit.m_hWnd!=NULL)
	{
		...
		//移动控件
		CSize SizeEdit;
		SizeEdit.SetSize(rcClient.Width()-m_uButtonWidth-7,rcClient.Height()-4);
		m_SkinComboBoxEdit.SetWindowPos(NULL,2,2,SizeEdit.cx,SizeEdit.cy,SWP_NOZORDER);
	}
