...
bool CAttemperEngineSink::OnTCPNetworkMainRegister(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
	case SUB_CS_C_CHECK_USER_ONLINE:
		{
			//效验数据
			ASSERT(wDataSize==sizeof(CMD_CS_C_CheckUserOnline));
			if (wDataSize!=sizeof(CMD_CS_C_CheckUserOnline)) return false;

			CMD_CS_C_CheckUserOnline* pCheckUserOnline = (CMD_CS_C_CheckUserOnline*)pData;

			CMD_CS_C_CheckUserOnlineRet CheckUserOnlineRet;
			ZeroMemory(&CheckUserOnlineRet,sizeof(CheckUserOnlineRet));


			CheckUserOnlineRet.dwUserID = pCheckUserOnline->dwUserID;
	...
	case SUB_CS_C_REGISTER_SERVER:	//注册房间
		{
			...
			lstrcpyn(GameServer.szServerPassWork,pRegisterServer->szServerPassWork,CountArray(GameServer.szServerPassWork));
	...
	case SUB_CS_C_SEARCH_CORRESPOND:	//协调查找
		{
			...
			if (pSearchCorrespond->szNickName[0]!=0)
			{
				...
				//查找用户
				CGlobalUserItem * pGlobalUserItem=m_GlobalInfoManager.SearchUserItemByNickName(pSearchCorrespond->szNickName);
				...
				if (pGlobalUserItem!=NULL)
				{
					...
					do
					{
						...
						//辅助信息
						SearchCorrespond.UserRemoteInfo[wIndex].cbGender=pGlobalUserItem->GetGender();
						SearchCorrespond.UserRemoteInfo[wIndex].cbMemberOrder=pGlobalUserItem->GetMemberOrder();
						SearchCorrespond.UserRemoteInfo[wIndex].cbMasterOrder=pGlobalUserItem->GetMasterOrder();

						//用户信息
						SearchCorrespond.UserRemoteInfo[wIndex].dwUserID=pGlobalUserItem->GetUserID();
						lstrcpyn(SearchCorrespond.UserRemoteInfo[wIndex].szNickName,pGlobalUserItem->GetNickName(),LEN_NICKNAME);
