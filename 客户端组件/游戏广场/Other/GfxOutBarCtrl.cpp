...
{
	crBackGroundColor = RGB(223,240,249);	//GetSysColor(COLOR_BACKGROUND);//RGB(0,128,128);//
	crBackGroundColor1 = RGB(223,240,249);	//GetSysColor(COLOR_BACKGROUND);
...
void CGfxOutBarCtrl::DrawFolder(CDC * pDC, const int iIdx, CRect rect, const bool bSelected)
{
	...
	CPngImage ImageFolder;
	ImageFolder.LoadImage(AfxGetInstanceHandle(),TEXT("GAME_ITEM_BACK_MID"));
	ImageFolder.DrawImage(pDC,rect.left,rect.top,rect.Width(),rect.Height(),0,0);
	if (!bSelected)
	{
...
void CGfxOutBarCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (ht == htFolder)
	{
		...
		bHigh = (iSelFolder==index)?true:false;
		if (::GetCapture() == NULL)
		{
			SetCapture();
...
void CGfxOutBarCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	_variant_t strSmallMenu, strLargeMenu, strGroupMenu, strFontMenu, strBackMenu;
	strSmallMenu.SetString(_ID_GFX_SMALLICON);
	cMenu.AppendMenu(MF_STRING, ID_GFX_SMALLICON, strSmallMenu.bstrVal);
	strLargeMenu.SetString(_ID_GFX_LARGEICON);
	cMenu.AppendMenu(MF_STRING, ID_GFX_LARGEICON, strLargeMenu.bstrVal);
	strGroupMenu.SetString(_ID_GFX_ADDAGROUP);
	cMenu.AppendMenu(MF_STRING, ID_GFX_GROUPICON, strGroupMenu.bstrVal);
	strFontMenu.SetString(_ID_GFX_FONTCOLOR);
	cMenu.AppendMenu(MF_STRING, ID_GFX_FONTCICON, strFontMenu.bstrVal);
	strBackMenu.SetString(_ID_GFX_BACKCOLOR);
	cMenu.AppendMenu(MF_STRING, ID_GFX_BACKCICON, strBackMenu.bstrVal);
	
	_variant_t	strRemove, strRename;
	strRemove.SetString(_ID_GFX_REMOVEITEM);
	strRename.SetString(_ID_GFX_RENAMEITEM);
	if (iHitInternal1 == htItem)
	{
		if (dwFlags&fRemoveItems || dwFlags&fEditItems)
		{
			...
			if (dwFlags&fRemoveItems)
				cMenu.AppendMenu(MF_STRING, ID_GFX_REMOVEITEM, strRemove.bstrVal);

			if (dwFlags&fEditItems)
				cMenu.AppendMenu(MF_STRING, ID_GFX_RENAMEITEM, strRename.bstrVal);
		}
	}
...
void CGfxOutBarCtrl::AnimateFolderScroll(const int iFrom, const int iTo)
{
	...
	if(bmpFrom.GetSafeHandle() == NULL)
		return;
	ASSERT(bmpFrom.GetSafeHandle() != NULL);
