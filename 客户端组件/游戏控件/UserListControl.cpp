	...
	case UD_NICKNAME:		//用户账号
		{
			...
			//获取变量
			LPCTSTR pszAccounts1=pIClientUserItem1->GetNickName();
			LPCTSTR pszAccounts2=pIClientUserItem2->GetNickName();
			
			//对比数据
			INT nResult=lstrcmp(pszAccounts1,pszAccounts2);
			return (bAscendSort==true)?nResult:-nResult;
		}
	case UD_LOOKON_USER:
		{
			//获取变量
// 			LPCTSTR pszAccounts1=pIClientUserItem1->GetAddrDescribe();
// 			LPCTSTR pszAccounts2=pIClientUserItem2->GetAddrDescribe();
			
			DOUBLE fScore1 = pIClientUserItem1->GetUserInfo()->lScore;
			DOUBLE fScore2 = pIClientUserItem2->GetUserInfo()->lScore;
			//对比数据
			INT nResult=fScore1>fScore2;//lstrcmp(pszAccounts1,pszAccounts2);
			return (bAscendSort==true)?nResult:-nResult;
...
VOID CUserListControl::ConstructString(VOID * pItemData, WORD wColumnIndex, LPTSTR pszString, WORD wMaxCount)
{
	...
	case UD_NICKNAME:		//用户昵称
		{
			...
			_sntprintf(pszString,wMaxCount,TEXT("%s****"),strNick.Left(4));
			break;
		}
	case UD_GENDER:			//用户性别
		{
	...
	case UD_SCORE:			//用户积分
		{
			_sntprintf(pszString,wMaxCount,SCORE_STRING,pIClientUserItem->GetUserScore());
			break;
		}
	case UD_EXPERIENCE:		//用户经验
		{
	...
	case UD_LOOKON_USER:	//旁观用户
		{
			_sntprintf(pszString,wMaxCount,TEXT("%.2lf"),pIClientUserItem->GetUserInfo()->lScore);
	//		lstrcpyn(pszString,pIClientUserItem->GetAddrDescribe(),wMaxCount);
// 			//无效处理
