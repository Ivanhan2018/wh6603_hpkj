...
LONGLONG CDataBaseAide::GetValue_LONGLONG(LPCTSTR pszItem)
{
	...
	DOUBLE DBvalue = DBVarValue;
	return (LONGLONG)DBvalue;
}
...
{
	ASSERT(m_pIDataBase!=NULL);
	LONG lSvalue = (LONG)lValue;
	m_pIDataBase->AddParameter(pszItem,adBigInt,ParameterDirection,sizeof(LONG),CDBVarValue((LONG)lSvalue));
	return;
}
