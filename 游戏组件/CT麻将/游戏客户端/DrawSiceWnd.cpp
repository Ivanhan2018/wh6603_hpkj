...
void CDrawSiceWnd::OnTimer(UINT nIDEvent)
{
	if(IDI_SICE_POS==nIDEvent)
	{
		...
		if(m_bSicePos>=17)
		{
			...
			Sleep(1000);
			if( m_pSinkWnd )
				m_pSinkWnd->SendEngineMessage(IDM_USER_ACTION_FINISH,NULL,NULL);
		}
		else
		{
			if(m_bSicePos==8)
			{
				Sleep(1000);
				if( m_pSinkWnd )
					m_pSinkWnd->SendEngineMessage(IDM_SICE_TWO,NULL,NULL);
			}
			m_bSicePos++;
		}
		Invalidate();
...
void CDrawSiceWnd::StopSicing(bool bFinish)
{
	if( IsWindowVisible() )
	{
		...
		if(bFinish)m_pSinkWnd->SendEngineMessage(IDM_USER_ACTION_FINISH,NULL,NULL);
