...
VOID CDlgInsureItem::DrawNumberString(CDC * pDC, SCORE lScore, INT nXPos, INT nYPos)
{
	...
	SwitchScoreFormat(lScore,3L,2L,szControl,CountArray(szControl));
	...
	if (nScoreLength>0L)
	{
		...
		//获取大小
		CSize SizeNumber;
		SizeNumber.SetSize(ImageNumber.GetWidth()/13L,ImageNumber.GetHeight());
		...
		for (INT i=0;i<nScoreLength;i++)
		{
			...
			if (szControl[i]==TEXT('-'))
			{
				ImageNumber.DrawImage(pDC,nXDrawPos,nYPos,SizeNumber.cx,SizeNumber.cy,SizeNumber.cx*12L,0L);
			}
			//绘画数字
...
VOID CDlgInsureItem::SwitchScoreString(SCORE lScore, LPTSTR pszBuffer, WORD wBufferSize)
{
	...
	_sntprintf(szSwitchScore,CountArray(szSwitchScore),TEXT("%I64d"),(LONGLONG)lScore);
...
//转换字符
VOID CDlgInsureItem::SwitchScoreFormat(SCORE lScore, UINT uSpace,UINT uDecimalCount,LPTSTR pszBuffer, WORD wBufferSize)
{
	//转换数值
	TCHAR szSwitchScore[16]=TEXT("");
	_sntprintf(szSwitchScore,CountArray(szSwitchScore),SCORE_STRING,lScore);
...
BOOL CDlgInsureSave::OnInitDialog()
{
	...
	m_ForgetInsure.SetBackGroundColor(CSkinDialog::m_SkinAttribute.m_crBackGround);
...
VOID CDlgInsureSave::OnBnClickedTakeScore()
{
	...
	if (lTakeScore<=0L)
	{
		...
		Information.ShowMessageBox(TEXT("取出的金币数量不能为空，请重新输入金币数量！"),MB_ICONERROR,30);
	...
	if (lTakeScore>m_pUserInsureInfo->lUserInsure)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("您保险柜金币的数目余额不足，请重新输入金币数量！"),MB_ICONERROR,30);
...
VOID CDlgInsureSave::OnBnClickedSaveScore()
{
	...
	if (lSaveScore<=0L)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("存入的金币数量不能为空，请重新输入金币数量！"),MB_ICONERROR,30);
	...
	if (lSaveScore>m_pUserInsureInfo->lUserScore)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("您所携带金币的数目余额不足，请重新输入金币数量！"),MB_ICONERROR,30);
...
VOID CDlgInsureTransfer::OnBnClickedTransferScore()
{
	...
	if (strNickName.GetLength()==0)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("请输入要赠送的玩家账号！"),MB_ICONERROR,30);
	...
	if (lTransferScore<=0L)
	{
		//提示消息
		CInformation Information(this);
		Information.ShowMessageBox(TEXT("赠送的金币数量不能为空，请重新输入金币数量！"),MB_ICONERROR,30);
	...
	//事件通知
	m_pDlgInsureMain->OnMissionStart();
	m_pDlgInsureMain->PerformTransferScore(strNickName,lTransferScore,szInsurePass);
...
BOOL CDlgInsureMain::OnInitDialog()
{
	...
	m_TabControl.InsertItem(INSURE_TRANSFER,TEXT(" 赠送金币 "));
...
VOID CDlgInsureMain::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	if (m_bInitInfo==true)
	{
		...
		//绘画提示
		LPCTSTR pszScoreString=TEXT("金  币：");
		...
		//构造提示
		/*TCHAR szString[128]=TEXT("");
		_sntprintf(szString,CountArray(szString),TEXT("温馨提示：存入金币免手续费，取出与赠送金币将扣除 %ld‰ 的手续费"),m_UserInsureInfo.wRevenue);
...
//转账事件
VOID CDlgInsurePlaza::PerformTransferScore(LPCTSTR pszNickName, SCORE lTransferScore, LPCTSTR pszInsurePass)
{
	//设置变量
	m_bSaveMission=false;
	m_bTakeMission=false;
...
//连接事件
bool CDlgInsurePlaza::OnEventMissionLink(INT nErrorCode,INT nSocketID)
{
	//连接结果
	if (nErrorCode!=0)
	{
		...
		if (m_bSaveMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("连接服务器超时，金币存入失败，请稍后再次重试！"),MB_OK|MB_ICONSTOP,30);
		}
		...
		if (m_bTakeMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("连接服务器超时，金币取出失败，请稍后再次重试！"),MB_OK|MB_ICONSTOP,30);
		}
		...
		if (m_bTransferMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("连接服务器超时，金币赠送失败，请稍后再次重试！"),MB_OK|MB_ICONSTOP,30);
		}
	...
	if (m_bTransferMission==true)
	{
		...
		CWHService::GetMachineID(UserTransferScore.szMachineID);
		lstrcpyn(UserTransferScore.szAccounts,m_szNickName,CountArray(UserTransferScore.szAccounts));
		lstrcpyn(UserTransferScore.szLogonPass,pGlobalUserData->szPassword,CountArray(UserTransferScore.szLogonPass));
		lstrcpyn(UserTransferScore.szInsurePass,m_szInsurePass,CountArray(UserTransferScore.szInsurePass));

		//发送数据
		m_MissionManager.SendData(MDM_GP_USER_SERVICE,SUB_GR_USER_TRANSFER_SCORE,&UserTransferScore,sizeof(UserTransferScore));
	}
...
bool CDlgInsurePlaza::OnEventMissionShut(BYTE cbShutReason)
{
	...
	if (cbShutReason!=SHUT_REASON_NORMAL)
	{
		...
		if (m_bSaveMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("与服务器的连接异常断开了，金币存入失败！"),MB_ICONERROR,30);
		}
		...
		if (m_bTakeMission==true)
		{
			//提示消息
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("与服务器的连接异常断开了，金币取出失败！"),MB_ICONERROR,30);
		}
...
//更新银行
VOID CDlgInsurePlaza::OnEventUpdateInsurePlaza(SCORE lCurrScore,SCORE lCurrInsure)
{
	//查询银行
	if(m_UserInsureInfo.lUserScore!=lCurrScore || m_UserInsureInfo.lUserInsure!=lCurrInsure)
	{
		PerformQueryInfo();
	}
...
//转账事件
VOID CDlgInsureServer::PerformTransferScore(LPCTSTR pszAccounts, SCORE lTransferScore, LPCTSTR pszInsurePass)
{
	//效验参数
	ASSERT(m_pITCPSocket!=NULL);
	if (m_pITCPSocket==NULL) return;
	...
	//设置变量
	TransferScoreRequest.lTransferScore=lTransferScore;
	lstrcpyn(TransferScoreRequest.szAccounts ,pszAccounts,CountArray(TransferScoreRequest.szAccounts));
	lstrcpyn(TransferScoreRequest.szLogonPass,pGlobalUserData->szPassword,CountArray(TransferScoreRequest.szLogonPass));
	lstrcpyn(TransferScoreRequest.szInsurePass,pszInsurePass,CountArray(TransferScoreRequest.szInsurePass));
...
//更新银行
VOID CDlgInsureServer::OnEventUpdateInsureServer(SCORE lCurrScore,SCORE lCurrInsure)
{
	//查询银行
	if(m_UserInsureInfo.lUserScore!=lCurrScore || m_UserInsureInfo.lUserInsure!=lCurrInsure)
	{
