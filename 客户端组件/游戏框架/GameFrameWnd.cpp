...
CGameFrameWnd::CGameFrameWnd()
{
	...
	EncircleFrame.pszImageGR=MAKEINTRESOURCE(IDB_FRAME_BR);
	...
	case IDC_MUTE:
		{
			...
			pGlobalUnits->m_bAllowBackGroundSound = !pGlobalUnits->m_bMuteStatuts;
			//状态处理
...
VOID CGameFrameWnd::RectifyControl(INT nWidth, INT nHeight)
{
	...
	{
		rcSplitter.left=m_nScrollXMax*77L/100L;
...
VOID CGameFrameWnd::OnSize(UINT nType, INT cx, INT cy) 
{
	...
	//滚动数据
	m_nScrollXMax=LESS_SCREEN_CX;
	m_nScrollYMax=LESS_SCREEN_CY;
