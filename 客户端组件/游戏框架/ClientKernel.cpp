...
IClientUserItem * CClientKernel::SearchUserByGameID(DWORD dwGameID)
{
	...
	//查找用户
	IClientUserItem * pIClientUserItem=m_GameUserManagerModule->SearchUserByUserID(dwGameID);
...
VOID CClientKernel::OnUserItemUpdate(IClientUserItem * pIClientUserItem, tagUserStatus & LastStatus)
{
	...
	if(m_pIStringMessage)
	{
		...
		{
			CString strNick;
			strNick.Format(L"%s",pIClientUserItem->GetUserInfo()->szNickName);
			strNick = strNick.Left(4);
			strNick+=_T("****");

			m_pIStringMessage->InsertUserOffLine(strNick);
		}
		else if(pIClientUserItem->GetUserStatus()==US_PLAYING && LastStatus.cbUserStatus==US_OFFLINE)
		{
			...
			_sntprintf(strMsg,CountArray(strMsg),TEXT("%s 断线回来了！"), strNick);
...
VOID CClientKernel::OnUserItemUpdate(IClientUserItem * pIClientUserItem, tagUserSpreadInfo & LastSpreadInfo)
{
	//效验状态
	ASSERT((m_UserEventSinkArray.GetCount()>0L)&&(pIClientUserItem!=NULL));
	if ((m_UserEventSinkArray.GetCount()==0L)||(pIClientUserItem==NULL)) return;
	...
	case IPC_SUB_GF_COLUMN_INFO:	//列表信息
		{
			...
			for (BYTE i=0;i<pColumnInfo->cbColumnCount;i++)
			{
				...
				//排除过虑
				if (pColumnInfo->ColumnItem[i].cbDataDescribe==UD_USER_ID) continue;
			...
			if (cbColumnCount<CountArray(ColumnItem))
			{
				...
				ColumnItem[cbIndex].cbColumnWidth=75;
				ColumnItem[cbIndex].cbDataDescribe=UD_NICKNAME;
				lstrcpyn(ColumnItem[cbIndex].szColumnName,TEXT("账户名称"),CountArray(ColumnItem[cbIndex].szColumnName));

				cbIndex=cbColumnCount++;
				ColumnItem[cbIndex].cbColumnWidth=80;
				ColumnItem[cbIndex].cbDataDescribe=UD_LOOKON_USER;
				lstrcpyn(ColumnItem[cbIndex].szColumnName,TEXT("游戏币"),CountArray(ColumnItem[cbIndex].szColumnName));
			}
	...
	case IPC_SUB_GF_USER_ENTER:		//用户进入
		{
			...
			{
				IClientUserItem* puseritem = m_GameUserManagerModule->GetTableUserItem(i);
				if(puseritem == NULL) continue;

				if(puseritem->GetUserID() == pUserInfoHead->dwUserID)
					return true;
			}
...
//			UserInfo.lLoveLiness=pUserInfoHead->lLoveLiness;
			...
			//激活用户
			m_GameUserManagerModule->ActiveUserItem(UserInfo,pIPCUserInfo->cbCompanion,szUserNote);
...
//	switch (wSubCmdID)
	...
	//		m_cbIsReceiveMsg = IniData.ReadInt(TEXT("PlazaUrl"),TEXT("IsReceiveMsg"),1);
	//		//提取数据
