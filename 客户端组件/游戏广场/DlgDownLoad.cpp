...
CDownLoadSink::CDownLoadSink()
{
	...
	for(int i = 0;i < 5;i++)
	{
		_sntprintf(szKey,CountArray(szKey), L"DownLoad%d", i);
		//读取配置
...
bool CDownLoadSink::PerformDownLoad(LPCTSTR pszClientName, WORD wKindID, WORD wServerID)
{
	...
	//构造地址
	TCHAR szDownLoadUrl[MAX_PATH]=TEXT("\\0");
	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/DownLoad.aspx?KindID=%ld"),szPlatformUrl[rand()%5],wKindID);
...
bool CDownLoadSink::PerformInstall()
{
	...
	bool	bIsHall=false;
	//判断是否大厅，如果是更新大厅，那么应该先退出
	CString strFileName = szTargetName;

	if(strFileName.Right(9).CompareNoCase(L"Plaza.exe") == 0)
	{
		bIsHall=true;
	}
	else if(strFileName.Right(8).CompareNoCase(L"Hall.exe") == 0)
	{
		bIsHall=true;
	}
	else if(strFileName.Right(10).CompareNoCase(L"Update.exe") == 0)
		bIsHall=true;
	...
	shExInfo.lpFile = strFileName;       // Application to start    
	shExInfo.lpParameters = NULL;                  // Additional parameters
	shExInfo.lpDirectory = szCommandLine;
	shExInfo.nShow = SW_SHOW;
	shExInfo.hInstApp = 0; 
	ShellExecuteEx(&shExInfo);

	m_ProcessInfoMation.hProcess = shExInfo.hProcess;
...
CDlgDownLoad::CDlgDownLoad() : CSkinDialog(IDD_DLG_DOWNLOAD)
{
	...
	m_btCancelOne.EnableWindow(TRUE);
	return;
}
...
BOOL CDlgDownLoad::OnInitDialog()
{
	...
	m_btRetryAgain.SetButtonColor(RGB(255,255,255));
	m_btCancelOne.SetButtonColor(RGB(255,255,255));
	m_btCancelAll.SetButtonColor(RGB(255,255,255));
	//设置图标
...
VOID CDlgDownLoad::OnBnClickedCancelOne()
{
	...
	if(m_DownLoadSinkArray.GetCount()<m_wFocusItem)
	{
		//构造地址
		TCHAR szDownLoadUrl[MAX_PATH]=TEXT("\\0");
		CDownLoadSink * pDownLoadSink=m_DownLoadSinkArray[0];

		_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/DownLoad.aspx?KindID=1"),pDownLoadSink->szPlatformUrl[rand()%5]);
		//	ShellExecute(NULL, _T("open"), _T("IEXPLORE"), szDownLoadUrl, NULL, SW_SHOWNORMAL);
		ShellExecute(NULL,TEXT("OPEN"),szDownLoadUrl,NULL,NULL,SW_NORMAL);
		return;
	}
	...
	//构造地址
	TCHAR szDownLoadUrl[MAX_PATH]=TEXT("\\0");
	if(pDownLoadSink->m_wKindID == 0)
	{
		pDownLoadSink->m_wKindID=1;
	}
	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/DownLoad.aspx?KindID=%ld"),pDownLoadSink->szPlatformUrl[rand()%5],pDownLoadSink->m_wKindID);
//	ShellExecute(NULL, _T("open"), _T("IEXPLORE"), szDownLoadUrl, NULL, SW_SHOWNORMAL);
	ShellExecute(NULL,TEXT("OPEN"),szDownLoadUrl,NULL,NULL,SW_NORMAL);
...
LRESULT CDlgDownLoad::OnItemStatusUpdateMessage(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		{
//			m_btRetryAgain.EnableWindow((VerdictRetryAgain(m_wFocusItem)==true)?TRUE:FALSE);
			m_btRetryAgain.EnableWindow(TRUE);
		}
	}
