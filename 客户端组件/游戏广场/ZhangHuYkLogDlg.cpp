...
void CZhangHuYkLogDlg::InitListCtrl()
{
	...
	m_listLog.InsertColumn(0, _T("用户名"), LVCFMT_CENTER, 120);
	m_listLog.InsertColumn(1, _T("类型"), LVCFMT_CENTER, 100);
	m_listLog.InsertColumn(2, _T("充值金额"), LVCFMT_CENTER, 96);
	m_listLog.InsertColumn(3, _T("取款金额"), LVCFMT_CENTER, 96);
	m_listLog.InsertColumn(4, _T("投注金额"), LVCFMT_CENTER, 96);
	m_listLog.InsertColumn(5, _T("返点金额"), LVCFMT_CENTER, 96);
	m_listLog.InsertColumn(6, _T("中奖金额"), LVCFMT_CENTER, 96);
	m_listLog.InsertColumn(7, _T("活动"), LVCFMT_CENTER, 96);
	m_listLog.InsertColumn(8, _T("盈亏"), LVCFMT_CENTER, 94);
	m_listLog.InsertColumn(9, _T("余额"), LVCFMT_CENTER, 93);
}
		...
		case SUB_GP_GET_YINGKUI_LOG_RET:
			{
				...
				for(int i = 0;i < wCount;i++)
				{
					...
					if(theAccount.type==1)
						m_listLog.SetItemText(item, 1, L"代理");
					else
						m_listLog.SetItemText(item, 1, L"个人");
...
// 			m_tEnd += CTimeSpan(0, 23, 59, 59);
			...
			pPlatformFrame->m_MissionManager.SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_YINGKUI_LOG_COUNT,&GetYingkuiLogCount,sizeof(GetYingkuiLogCount));
			return;
		}
...
// 			}
			...
			CPlatformFrame *pPlatformFrame = CPlatformFrame::GetInstance();
			pPlatformFrame->m_MissionManager.SendData(MDM_GP_USER_SERVICE,SUB_GP_GET_YINGKUI_LOG,&YingkuiLog,sizeof(YingkuiLog));
			return;
		}
	}
