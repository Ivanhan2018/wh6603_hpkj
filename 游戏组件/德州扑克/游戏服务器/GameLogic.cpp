...
//查找最大
bool CGameLogic::SelectMaxUser(BYTE bCardData[GAME_PLAYER][MAX_CENTERCOUNT],UserWinList &EndResult,const DZPKSCORE lAddScore[])//zrh
{
	//清理数据
	...
	if(EndResult.bSameCount>1 && lAddScore!=NULL)
	{
		...
		while((iSameCount--)>0)
		{
			DZPKSCORE lTempSocre = lAddScore[EndResult.wWinerList[iSameCount]];
