...
bool CServiceUnits::InitializeService()
{
	...
	for (INT i=0;i<CountArray(m_DataBaseEngineSink);i++)
	{
		...
		m_AttemperEngineSink.m_DataBaseEngineSink[i] = &m_DataBaseEngineSink[i];
	}
...
bool CServiceUnits::SetServiceStatus(enServiceStatus ServiceStatus)
{
	...
	if (m_ServiceStatus!=ServiceStatus)
	{
		...
		{
			CString strLog;
			strLog.Format(L"登录服务器版本:%ld.%ld.%ld  -- [ 启动失败 ]", m_AttemperEngineSink.m_pInitParameter->m_dwMainVer, m_AttemperEngineSink.m_pInitParameter->m_dwSubVer,m_AttemperEngineSink.m_pInitParameter->m_dwProductVer);
	...
	case UI_CORRESPOND_RESULT:		//协调结果
		{
			...
			if ((m_ServiceStatus!=ServiceStatus_Service)&&(pControlResult->cbSuccess==ER_SUCCESS))
			{
				...
				m_AttemperEngineSink.OnGetNews();
				m_AttemperEngineSink.OnGetMapBonusRet();
			}
