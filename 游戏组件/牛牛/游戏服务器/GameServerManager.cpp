...
//////机器定义
#ifndef _DEBUG
#define ANDROID_SERVICE_DLL_NAME	TEXT("OxAndroid.dll")	//组件名字
#else
#define ANDROID_SERVICE_DLL_NAME	TEXT("OxAndroid.dll")	//组件名字
#endif
...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szServerDLLName,TEXT("OxServer.DLL"),CountArray(m_GameServiceAttrib.szServerDLLName));
	lstrcpyn(m_GameServiceAttrib.szClientEXEName,TEXT("Ox.EXE"),CountArray(m_GameServiceAttrib.szClientEXEName));
...
{
	//修改参数
	GameServiceOption.lCellScore=__max(0.1,GameServiceOption.lCellScore);

	if (GameServiceOption.wServerType&GAME_GENRE_GOLD)
	{
		//积分下限
		GameServiceOption.lMinEnterScore=__max(GameServiceOption.lMinEnterScore,GameServiceOption.lCellScore*15);

	}

	//输分限制(原来的积分上限)
	if (GameServiceOption.lRestrictScore>COMPARE_ZERO||GameServiceOption.lRestrictScore<-COMPARE_ZERO)
	{
