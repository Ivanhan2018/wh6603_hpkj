...
BOOL CDlgEnquire::OnInitDialog()
{
	...
	//m_btCloseServer.EnableWindow((pPlatformFrame->m_ServerViewItem.m_hWnd!=NULL)?TRUE:FALSE);
	...
	m_btCancel.SetWindowPos(NULL,SizeWindow.cx-rcCancel.Width()-19,4,0,0,SWP_NOZORDER|SWP_NOSIZE|SWP_NOREDRAW);
	...
	m_btCloseServer.SetWindowPos(NULL,LAYERED_SIZE+nButtonSpace*3+rcButton.Width()*2+24,134,0,0,SWP_NOZORDER|SWP_NOSIZE|SWP_NOREDRAW);
	m_btSwitchAccounts.SetWindowPos(NULL,LAYERED_SIZE+nButtonSpace*2+rcButton.Width()*1+62,134,0,0,SWP_NOZORDER|SWP_NOSIZE|SWP_NOREDRAW);
	m_btClosePlaza.SetWindowPos(NULL,LAYERED_SIZE+nButtonSpace*1+rcButton.Width()*0+99,134,0,0,SWP_NOZORDER|SWP_NOSIZE|SWP_NOREDRAW);

	//m_btCloseServer.ShowWindow(SW_HIDE);
	//广告控件
...
//	CFont* pOldFont = pBufferDC->SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont());
