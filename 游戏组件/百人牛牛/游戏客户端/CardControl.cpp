...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if(m_blGameEnd)		///普通时候。和游戏结束时候
		{
				m_ImageCard.TransDrawImage(pDC,m_ptSrc.x + (OriginPoint.x+nXDrawPos+15 - m_ptSrc.x)/8 * m_nFrameIndex[i],m_ptSrc.y + (OriginPoint.y+nYDrawPos - m_ptSrc.y )/8 * m_nFrameIndex[i],
					m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
