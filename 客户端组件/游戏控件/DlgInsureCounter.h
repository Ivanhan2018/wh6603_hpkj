...
//消息定义
#define WM_INSUREPLAZA_UPDATA      (WM_USER+500)					    //银行更新消息
...
interface IInsureCounterAction
{
	...
	//转账事件
	virtual VOID PerformTransferScore(LPCTSTR pszNickName, SCORE lTransferScore, LPCTSTR pszInsurePass)=NULL;
};
...
protected:
	SCORE							m_lScore;							//金币数目
	TCHAR							m_szNickName[LEN_NICKNAME];			//用户账号
...
protected:
	...
	//转账事件
	virtual VOID PerformTransferScore(LPCTSTR pszNickName, SCORE lTransferScore, LPCTSTR pszInsurePass);
...
public:
	//更新银行
	VOID OnEventUpdateInsurePlaza(SCORE lCurrScore,SCORE lCurrInsure);
};
...
protected:
	...
	virtual VOID PerformTakeScore(SCORE lTakeScore, LPCTSTR pszInsurePass);
	//转账事件
	virtual VOID PerformTransferScore(LPCTSTR pszAccounts, SCORE lTransferScore, LPCTSTR pszInsurePass);
...
public:
	//更新银行
	VOID OnEventUpdateInsureServer(SCORE lCurrScore,SCORE lCurrInsure);
};
