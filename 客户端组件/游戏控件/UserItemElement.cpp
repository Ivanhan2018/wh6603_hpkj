...
VOID CUserItemElement::ConstructNameCard(IClientUserItem * pIClientUserItem, IGameLevelParser * pIGameLevelParser, CImage&ImageNameCard)
{
	...
	//位置定义
	CRect rcAccounts(60,5,ImageSize.cx-5,5+12);
	...
	_sntprintf(szString,CountArray(szString),TEXT("账号：%s [ %s ]"),pIClientUserItem->GetNickName(),pszUserOrder);
	pDC->DrawText(szString,lstrlen(szString),&rcAccounts,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
...
bool CUserItemElement::EfficacyNickName(LPCTSTR pszNickName, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if (nCharLength<LEN_LESS_NICKNAME)
	{
		...
		{
			_sntprintf(szDescribe,wMaxCount,TEXT("游戏账号必须大于 %ld 位以上，请重新输入！"),LEN_LESS_NICKNAME);
			return false;
		}
	}
...
bool CUserItemElement::EfficacyPassPortID(LPCTSTR pszPassPortID, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	//如果是空返回真,不作认证 身份证是可选的
	if(nCharLength==0) return true;
...
//效验QQ
bool CUserItemElement::EfficacyQQ(LPCTSTR QQ, TCHAR szDescribe[], WORD wMaxCount)
{
	//变量定义
	//bool bNumber=true;
	//获取长度
	UINT nCharLength=lstrlen(QQ);


	//长度判断
	if (nCharLength<4)
	{
		_sntprintf(szDescribe,wMaxCount,TEXT("您的QQ号码输入有误，请重新输入！"),LEN_LESS_PASSWORD);
		return false;
	}
	...
	for (UINT i=0;i<nCharLength;i++)
	{
		if ((QQ[i]<TEXT('0'))||(QQ[i]>TEXT('9')))
		{
				lstrcpyn(szDescribe,TEXT("QQ号码必须全为数字，请重新输入！"),wMaxCount);
				return false;
		}
	}
...
//效验手机
bool CUserItemElement::EfficacyIPHONE(LPCTSTR IPHONE, TCHAR szDescribe[], WORD wMaxCount)
{
	//变量定义
	//bool bNumber=true;
	//获取长度
	UINT nCharLength=lstrlen(IPHONE);
	...
	if (nCharLength<11)
	{
		_sntprintf(szDescribe,wMaxCount,TEXT("您的手机号码输入有误，请重新输入！"),LEN_LESS_PASSWORD);
		return false;
	}
	...
	for (UINT i=0;i<nCharLength;i++)
	{
		if ((IPHONE[i]<TEXT('0'))||(IPHONE[i]>TEXT('9')))
		{
				lstrcpyn(szDescribe,TEXT("您的手机号码输入有误，请重新输入！"),wMaxCount);
				return false;
		}
	}
...
//效验EMAIN
bool CUserItemElement::EfficacyEmain(LPCTSTR Emain, TCHAR szDescribe[], WORD wMaxCount)
{
	//变量定义
	bool bs1=false;
	bool bs2=false;
	//获取长度
	UINT nCharLength=lstrlen(Emain);
	...
	for (UINT i=0;i<nCharLength;i++)
	{
		...
		//EWIN网络网络科技，判断是否出现过符号
		if (( bs1==false || bs2==false ))
		{
				lstrcpyn(szDescribe,TEXT("您输入的E-Main帐号格式不正确，请重新输入！"),wMaxCount);
				return false;
		}
