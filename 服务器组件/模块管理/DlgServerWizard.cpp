...
BOOL CDlgServerOptionItem1::OnInitDialog()
{
	...
	//关联控件
	m_CellScoreEdit.Attach(GetDlgItem(IDC_CELL_SCORE)->m_hWnd);
	m_CellScoreEdit.SetLimitText(12);
	...
	//EWIN网络密码房间
	if (m_pGameServiceOption->szServerPassWork[0]==0)
	{
		SetDlgItemText(IDC_SERVER_PASSWORK,m_pGameServiceAttrib->szServerPassWork);
	}
	else SetDlgItemText(IDC_SERVER_PASSWORK,m_pGameServiceOption->szServerPassWork);


	//密码房间是否启用
	if (m_pGameServiceOption->wServerKind!=0)
	{
		((CButton*)GetDlgItem(IDC_SERVER_KIND))->SetCheck(TRUE); 
	}
	else ((CButton*)GetDlgItem(IDC_SERVER_KIND))->SetCheck(FALSE); 

	//虚拟人数
	if (m_pGameServiceOption->wVirtualUserCount!=0)
	{
		SetDlgItemInt(IDC_VIRTUAIUSERCOUNT,m_pGameServiceOption->wVirtualUserCount);
	}
	else SetDlgItemInt(IDC_VIRTUAIUSERCOUNT,0);
	...
	//房间配置
	if (m_pGameServiceOption->lCellScore!=0.0) SetDlgItemDouble(IDC_CELL_SCORE,(SCORE)m_pGameServiceOption->lCellScore);
...
bool CDlgServerOptionItem1::SaveInputInfo()
{
	...
	m_pGameServiceOption->lCellScore=GetDlgItemDouble(IDC_CELL_SCORE);
	...
	//EWIN网络密码房间
	m_pGameServiceOption->wServerKind=((CButton*)GetDlgItem(IDC_SERVER_KIND))->GetCheck();
	GetDlgItemText(IDC_SERVER_PASSWORK,m_pGameServiceOption->szServerPassWork,CountArray(m_pGameServiceOption->szServerPassWork));

	//虚拟人数
	m_pGameServiceOption->wVirtualUserCount=GetDlgItemInt(IDC_VIRTUAIUSERCOUNT);
	...
	//EWIN网络密码房间
	if (m_pGameServiceOption->wServerKind==1){
		if (m_pGameServiceOption->szServerPassWork[0]==0)
		{
			AfxMessageBox(TEXT("房间密码不能为空，请输入游戏房间密码"),MB_ICONERROR);
			return false;
		}
	}
...
VOID CDlgServerOptionItem1::InitCtrlWindow()
{
	...
	//EWIN网络密码房间
	((CEdit *)GetDlgItem(IDC_SERVER_PASSWORK))->LimitText(31);

	//虚拟人数
	((CEdit *)GetDlgItem(IDC_VIRTUAIUSERCOUNT))->LimitText(3);
...
VOID CDlgServerOptionItem1::UpdateDataBaseName()
{
	...
	case GAME_GENRE_EDUCATE: //练习类型 
		{
			SetDlgItemText(IDC_DATABASE_NAME,szEducateDB);
			break;
		}
	default:
		{
...
//设置数值
VOID CDlgServerOptionItem1::SetDlgItemDouble(int id,double Value)
{
	//变量定义
	CString  szText=TEXT("");
	szText.Format(SCORE_STRING,Value);

	//设置数值
	SetDlgItemText(id,szText);
}

//获取数值
double CDlgServerOptionItem1::GetDlgItemDouble(int id)
{
	//变量定义
	CString szText=TEXT("");
	GetDlgItemText(id,szText);

	double dResult=0;
#ifdef _UNICODE
	dResult=_wtof(szText.GetBuffer());
	szText.ReleaseBuffer();
#else 
	atof(szText.GetBuffer());
	szText.ReleaseBuffer();
#endif
...
BOOL CDlgServerOptionItem2::OnInitDialog()
{
	...
	//隐藏信息
	bool bAvertCheatMode=CServerRule::IsAllowAvertCheatMode(m_pGameServiceOption->dwServerRule);
	...
	{
		bool bDynamicJoin=CServerRule::IsAllowDynamicJoin(m_pGameServiceOption->dwServerRule);
	...
	{
		bool bAllowAndroidUser=CServerRule::IsAllowAndroidAttend(m_pGameServiceOption->dwServerRule);
	...
	{
		bool bOffLineTrustee=CServerRule::IsAllowOffLineTrustee(m_pGameServiceOption->dwServerRule);
...
bool CDlgServerOptionItem2::SaveInputInfo()
{
	...
	if (m_pGameServiceAttrib->cbDynamicJoin==TRUE)
	{
		...
		CServerRule::SetAllowDynamicJoin(m_pGameServiceOption->dwServerRule,(pDynamicJoin->GetCheck()==BST_CHECKED));
	}
	...
	if (m_pGameServiceAttrib->cbOffLineTrustee==TRUE)
	{
		...
		CServerRule::SetAllowOffLineTrustee(m_pGameServiceOption->dwServerRule,(pOffLineTrustee->GetCheck()==BST_CHECKED));
	}
...
BOOL CDlgServerOptionItem3::OnInitDialog()
{
	...
	//游戏最少人数
	if (m_pGameServiceOption->wMinGameUser!=0)
	{
		SetDlgItemInt(IDC_DISTRIBUTE_START_MIN_USER,m_pGameServiceOption->wMinGameUser);
	}

	//游戏最大人数
	if (m_pGameServiceOption->wMaxGameUser!=0)
	{
		SetDlgItemInt(IDC_DISTRIBUTE_START_MAX_USER,m_pGameServiceOption->wMaxGameUser);
	}
...
bool CDlgServerOptionItem3::SaveInputInfo()
{
	...
	m_pGameServiceOption->wMinGameUser=GetDlgItemInt(IDC_DISTRIBUTE_START_MIN_USER);
	m_pGameServiceOption->wMaxGameUser=GetDlgItemInt(IDC_DISTRIBUTE_START_MAX_USER);
...
BOOL CDlgServerWizardItem2::OnInitDialog()
{
	...
	m_TabCtrl.InsertItem(ITEM_SERVER_OPTION_3,TEXT("分组设置"));
...
VOID CDlgServerWizard::OnBnClickedFinish()
{
	...
	lstrcpyn(GameServerCreate.szServerPassWork,m_ModuleInitParameter.GameServiceOption.szServerPassWork,CountArray(GameServerCreate.szServerPassWork));


	//虚拟人数
	GameServerCreate.wVirtualUserCount=m_ModuleInitParameter.GameServiceOption.wVirtualUserCount;
	...
	GameServerCreate.wMinGameUser=m_ModuleInitParameter.GameServiceOption.wMinGameUser;
	GameServerCreate.wMaxGameUser=m_ModuleInitParameter.GameServiceOption.wMaxGameUser;	
	...
	//EWIN网络密码房间
	m_ModuleInitParameter.GameServiceOption.wServerKind=GameServerResult.wServerKind;
	lstrcpyn(m_ModuleInitParameter.GameServiceOption.szServerPassWork,GameServerResult.szServerPassWork,LEN_SERVER);

	//虚拟人数
	m_ModuleInitParameter.GameServiceOption.wVirtualUserCount=GameServerResult.wVirtualUserCount;
	...
	m_ModuleInitParameter.GameServiceOption.wMinGameUser=GameServerResult.wMinGameUser;
	m_ModuleInitParameter.GameServiceOption.wMaxGameUser=GameServerResult.wMaxGameUser;
	...
	if(pMsg->message==WM_CHAR) 
	{
		...
		if(pMsg->wParam<=0xff)
		{
			...
			if(!isdigit(ch)) 
			{
				...
				{
					if( szWindowText.Find(TEXT(".")) == -1 && szWindowText!=TEXT("")) 
						bReturn=FALSE;
				}
				else
				{	
					...
					if(pMsg->wParam==VK_BACK)
						bReturn=CEdit::PreTranslateMessage(pMsg);
					else
						bReturn=TRUE;
				}
			}
			...
			{
				//定义变量
				DWORD nSelPos=LOWORD(GetSel()); 
				int nPointPos=szWindowText.Find(TEXT("."));

				//位数判断
				if(nPointPos!=-1 && nSelPos>nPointPos && ((szWindowText.GetLength()-nPointPos)>m_cbDecimalCount)) 
				//EWIN网络修正
				//if(nPointPos!=-1 && (int)nSelPos>nPointPos && ((szWindowText.GetLength()-nPointPos)>m_cbDecimalCount)) 
					bReturn=TRUE;
				else
					bReturn=CEdit::PreTranslateMessage(pMsg); 
			}
