...
//增加用户
IClientUserItem * CPlazaUserManager::ActiveUserItem(tagUserInfo & UserInfo)
{
	//变量定义
	...
	if (pCompanionInfo!=NULL)
	{
		//一致判断
		bool bSameData=true;
		if (lstrcmp(UserInfo.szNickName,pCompanionInfo->szNickName)!=0) bSameData=false;
...
//扩展信息
bool CPlazaUserManager::UpdateUserItemSpreadInfo(IClientUserItem * pIClientUserItem, tagUserSpreadInfo * const pUserSpreadInfo)
{
	//效验参数
	ASSERT(pUserSpreadInfo!=NULL);
	ASSERT(pIClientUserItem!=NULL);
	...
	CopyMemory(UserSpreadInfo.szIpAddrDescribe,pUserInfo->szAddrDescribe,sizeof(UserSpreadInfo.szIpAddrDescribe));

	//设置数据
	CopyMemory(pUserInfo->szAddrDescribe,pUserSpreadInfo->szIpAddrDescribe,sizeof(pUserInfo->szAddrDescribe));

	//通知更新
	ASSERT(m_pIUserManagerSink!=NULL);
	m_pIUserManagerSink->OnUserItemUpdate(pIClientUserItem,UserSpreadInfo);
...
//增加用户
IClientUserItem * CGameUserManager::ActiveUserItem(tagUserInfo & UserInfo, BYTE cbCompanion, LPCTSTR pszUserNote)
{
	//效验状态
...
//更新状态
bool CGameUserManager::UpdateUserItemSpreadInfo(IClientUserItem * pIClientUserItem, tagUserSpreadInfo * const pUserSpreadInfo)
{
	//效验参数
	ASSERT(pUserSpreadInfo!=NULL);
	ASSERT(pIClientUserItem!=NULL);
