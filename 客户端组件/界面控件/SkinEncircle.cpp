...
bool CEncircleBMP::LoadEncircleImage(tagEncircleBMP & EncircleImage)
{
	...
	if (m_hResInstance==NULL)
	{
		...
		EncircleImage.ImageGR.Load(m_EncircleResource.pszImageGR);
	}
	else
	{
		//资源加载
		...
		EncircleImage.ImageGR.LoadFromResource(m_hResInstance,m_EncircleResource.pszImageGR);
	}
...
bool CEncircleBMP::InitDefaultResource()
{
	...
	EncircleResource.pszImageGR=MAKEINTRESOURCE(IDB_ENCIRCLE_GR);
...
bool CEncircleBMP::DrawEncircleFrame(CDC * pDC, CRect & rcEncircle)
{
	...
	pSkinRenderManager->RenderImage(EncircleImage.ImageGR);
	...
	CSize SizeImageGR;
	SizeImageGR.SetSize(EncircleImage.ImageGR.GetWidth(),EncircleImage.ImageGR.GetHeight());
	//绘画上边
	...
	for (INT nYPos=rcEncircle.top+SizeImageTL.cy;nYPos<nYTerminate;nYPos+=SizeImageGR.cy)
	{
		for (INT nXPos=rcEncircle.left+SizeImageTL.cx;nXPos<nXTerminate;nXPos+=SizeImageGR.cx)
		{
			EncircleImage.ImageGR.BitBlt(pDC->m_hDC,nXPos,nYPos,__min(nXTerminate-nXPos,SizeImageGR.cx),__min(nYTerminate-nYPos,SizeImageGR.cy),0,0);
		}
	}
