...
public:
	...
	bool EfficacyPassPortID(LPCTSTR pszPassPortID, TCHAR szDescribe[], WORD wMaxCount);
		//效验QQ
	bool EfficacyQQ(LPCTSTR QQ, TCHAR szDescribe[], WORD wMaxCount);
		//效验EMAIN
	bool EfficacyEmain(LPCTSTR Emain, TCHAR szDescribe[], WORD wMaxCount);
		//效验手机
	bool EfficacyIPHONE(LPCTSTR IPHONE, TCHAR szDescribe[], WORD wMaxCount);
	//静态函数
public:
	//获取对象
