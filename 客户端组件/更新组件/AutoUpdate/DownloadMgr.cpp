...
BOOL CDownloadMgr::AddTask(LPCTSTR pszRemoteFile, LPCTSTR pszLocalFile)
{
	...
	CString strLog;
	strLog.Format(_T("\n 本地文件:%s"),pDownloadTask->szLocalFile);
...
DWORD CDownloadMgr::OnDownloadProc()
{
	...
	try
	{
		...
		for (INT_PTR i = 0; i < nCount; i++)
		{
			...
			CString strURL=pDownloadTask->szRemoteFile;

			CString strLog;
			strLog.Format(_T("\n 远程文件URL:%s"),strURL);
			OutputDebugString(strLog);

			pHttpFile=(CHttpFile*)m_pInteSession->OpenURL(strURL.GetString(), 1, 
			...
			if (memcmp(szErrorHtml1,buffer,_tcslen(szErrorHtml1))==0) throw 0;
			if (memcmp(szErrorHtml2,buffer,_tcslen(szErrorHtml2))==0) throw 0;
			...
			CString strLocal=pDownloadTask->szLocalFile;
			...
			// Create local file
			_stprintf(szLocalPath, _T("%s.dtm"),pDownloadTask->szLocalFile);
	...
	catch(...)
	{
		...
		TCHAR szError[512]={0};
		_stprintf(szError, _T("下载 %s 文件失败"), pDownloadTask->szRemoteFile);
		m_nStatus = DMS_ERROR;
		m_pDownloadSink->OnStatusCallback(0, 0, DMS_ERROR, szError);
		return 1;
	}
...
DWORD CDownloadMgr::OnSetupFilesProc()
{
	...
	int nCount = m_DownloadList.GetCount();
	for (int i = 0; i < nCount; i++)
	{
		...
		//复制文件
		_stprintf(szSrcFile, _T("%s.dtm"), pDownloadTask->szLocalFile);
		_stprintf(szDesFile, _T("%s"), pDownloadTask->szLocalFile);
