...
//链接文件
	...
	#pragma comment(lib,"Compress\\lzma.lib")
#else
	#pragma comment(lib,"Version")
	#pragma comment(lib,"Compress\\lzma.lib")
#endif
...
ULONG CWHService::CompressData(LPBYTE pcbSourceData, ULONG lSourceSize, BYTE cbResultData[], ULONG lResultSize)
{
	...
	int nRet = LzmaCompress(cbResultData, &lDestSize, pcbSourceData, lSourceSize, prop,
		&sizeProp, 9, (1 << 24), 3, 0, 2, 32, 2);
	if(nRet == SZ_OK)
	{
		if(lSourceSize > 4)
		{
			strLog.Format(L"WBJ: compress222 from %ld to %ld, data:%c,%c", lSourceSize, lDestSize, pcbSourceData,pcbSourceData+1);
	...
	{
			strLog.Format(L"WBJ:ERROR compress222 from %ld to %ld, data:%c,%c", lSourceSize, lDestSize, pcbSourceData,pcbSourceData+1);
			OutputDebugString(strLog);
...
ULONG CWHService::UnCompressData(LPBYTE pcbSourceData, ULONG lSourceSize, BYTE cbResultData[], ULONG lResultSize)
{
	...
	int nRet =  LzmaUncompress(cbResultData, &Size1, pcbSourceData, &Size2, prop, 5);
	return Size1;
}
