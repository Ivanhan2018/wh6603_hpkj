...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	//获取接口
	CGameClientView * pGameClientView = (CGameClientView * )GetParent();
	if(pGameClientView==NULL)return;
	...
	if(dwShootCount==2)
	{
		...
		{
			bHaveOx = true;
			pGameClientView->PostEngineMessage(IDM_OX_ENABLE,1,0);
		}
	}
	else if(dwShootCount==3)
	{
		...
		{
			bHaveOx = true;
			pGameClientView->PostEngineMessage(IDM_OX_ENABLE,1,1);
		}
	}

	if(!bHaveOx)pGameClientView->PostEngineMessage(IDM_OX_ENABLE,0,0);
