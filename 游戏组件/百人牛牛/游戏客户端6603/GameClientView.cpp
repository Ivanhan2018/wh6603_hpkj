...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	m_ImageTimeFlag.BitBlt(pDC->GetSafeHdc(), nWidth/2-348+49, nHeight/2+225-372+10, nTimeFlagWidth, m_ImageTimeFlag.GetHeight(),
	...
	if (wUserTimer!=0) DrawUserClock(pDC,nWidth/2-323+49,nHeight/2+197-372,wUserTimer);
...
{	
	...
	if (true==bSet)
	{
		...
		if (false==bWinTian && false==bWinDi && false==bWinXuan &&false ==bWinHuang )
		{
			...
			m_ImageFrameDiMen.LoadFromResource(this,hInstance,IDB_FRAME_DI_EMN);
			m_ImageFrameHuangMen.LoadFromResource(this,hInstance,IDB_FRAME_HUANG_EMN);
			m_ImageFrameXuanMen.LoadFromResource(this,hInstance,IDB_FRAME_XUAN_EMN);
...
void CGameClientView::DrawType(CDC* pDC,WORD wChairID)
{
	...
	for (int i = 0;i<5;i++)	
	{
		...
		m_ImageCardType[i].TransDrawImage(pDC,m_CardTypePoint[i].x,m_CardTypePoint[i].y,m_ImageCardType[i].GetWidth()/14,m_ImageCardType[i].GetHeight(),m_ImageCardType[i].GetWidth()/14*iIndex,0,RGB(255,0,255));
	}
}
...
void CGameClientView::OnTimer(UINT nIDEvent)
{
	...
	if(IDI_POSTCARD ==nIDEvent)
	{
		...
		if(m_bcShowCount<5)
		{
			...
			//MyDebug(_T("Client::View::Time %d"), m_bcShowCount);
			if (m_bcShowCount == 2)
