...
#define X_RADIUS					150									//绘画最大宽
#define	Y_RADIUS					40									//绘画最大高
...
CJettonControl::CJettonControl()
{
	...
	m_nJettonWidth = m_PngJetton.GetWidth()/14;
	m_lJetonIndex[0] =1;
	m_lJetonIndex[1] =2;
	m_lJetonIndex[2] =5;
	m_lJetonIndex[3] =10;
...
VOID CJettonControl::SetCellScore(LONGLONG sCellScore)
{
	m_lJetonIndex[0] =sCellScore;
	m_lJetonIndex[1] =2*sCellScore;
	m_lJetonIndex[2] =5*sCellScore;
	m_lJetonIndex[3] =10*sCellScore;
	m_lJetonIndex[4] =20*sCellScore;
	m_lJetonIndex[5] =50*sCellScore;
	m_lJetonIndex[6] =100*sCellScore;
	m_lJetonIndex[7] =200*sCellScore;
	m_lJetonIndex[8] =500*sCellScore;
	m_lJetonIndex[9] =1000*sCellScore;
}
//重置控件
...
{	
	...
	for( ; ; )
	{
		...
		//过滤0分筹码,和分数与第一个相反的筹码
		if((jp.lScore==0) || (lFirstScore>0&&jp.lScore<0) || (lFirstScore<0L&&jp.lScore>0)
			) 
		{
			if( jp.lScore==0 )
...
VOID CJettonControl::CompactJetIndex()
{
	...
	do
	{
		...
		for( int i = MAX_JETTON_INDEX-1; i >= 0; i-- )
		{
			...
			if( lScore <=m_lJetonIndex[0] ) break;
		}
