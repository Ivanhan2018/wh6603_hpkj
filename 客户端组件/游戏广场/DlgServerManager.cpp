...
BOOL CDlgServerManager::OnInitDialog()
{
	...
	//会员配置
	LPCTSTR pszMember[]={TEXT("没有限制"),TEXT("新手账号"),TEXT("普通账号"),TEXT("中级账号"),TEXT("高级账号")};
...
VOID CDlgServerManager::OnOK()
{
	...
	CServerRule::SetAllowAvertCheatMode(ServerOption.ServerOptionInfo.dwServerRule,(pAvertCheatMode->GetCheck()==BST_CHECKED));
	...
	CServerRule::SetAllowDynamicJoin(ServerOption.ServerOptionInfo.dwServerRule,(pDynamicJoin->GetCheck()==BST_CHECKED));

	//断线代打
	CButton * pOffLineTrustee=(CButton *)GetDlgItem(IDC_OFFLINE_TRUSTEE);
	CServerRule::SetAllowOffLineTrustee(ServerOption.ServerOptionInfo.dwServerRule,(pOffLineTrustee->GetCheck()==BST_CHECKED));

	//允许机器
	CButton * pAllowAndroidUser=(CButton *)GetDlgItem(IDC_ALLOW_ANDROID_USER);
	CServerRule::SetAllowAndroidAttend(ServerOption.ServerOptionInfo.dwServerRule,(pAllowAndroidUser->GetCheck()==BST_CHECKED));
	...
	{
		CInformation Information;
		Information.ShowMessageBox(TEXT("进入房间最低账号级别比最高账号级别还高，将会造成任何玩家都不能进入"),MB_ICONERROR);
		return;
	}
...
VOID CDlgServerManager::SetServerOptionInfo(tagServerOptionInfo & ServerOptionInfo, DWORD dwRuleMask)
{
	...
	((CButton *)GetDlgItem(IDC_AVERT_CHEAT_MODE))->EnableWindow(CServerRule::IsAllowAvertCheatMode(dwRuleMask));
	((CButton *)GetDlgItem(IDC_AVERT_CHEAT_MODE))->SetCheck((bAllowAvertCheatMode==TRUE)?BST_CHECKED:BST_UNCHECKED);
	...
	((CButton *)GetDlgItem(IDC_DYNAMIC_JOIN))->EnableWindow(CServerRule::IsAllowDynamicJoin(dwRuleMask));
	((CButton *)GetDlgItem(IDC_DYNAMIC_JOIN))->SetCheck((bAllowDynamicJoin==TRUE)?BST_CHECKED:BST_UNCHECKED);

	//允许机器
	bool bAllowAndroidAttend=CServerRule::IsAllowAndroidAttend(ServerOptionInfo.dwServerRule);
	((CButton *)GetDlgItem(IDC_ALLOW_ANDROID_USER))->EnableWindow(CServerRule::IsAllowAndroidAttend(dwRuleMask));
	((CButton *)GetDlgItem(IDC_ALLOW_ANDROID_USER))->SetCheck((bAllowAndroidAttend==TRUE)?BST_CHECKED:BST_UNCHECKED);

	//断线代打
	bool bAllowOffLineTrustee=CServerRule::IsAllowOffLineTrustee(ServerOptionInfo.dwServerRule);
	((CButton *)GetDlgItem(IDC_OFFLINE_TRUSTEE))->EnableWindow(CServerRule::IsAllowOffLineTrustee(dwRuleMask));
	((CButton *)GetDlgItem(IDC_OFFLINE_TRUSTEE))->SetCheck((bAllowOffLineTrustee==TRUE)?BST_CHECKED:BST_UNCHECKED);
