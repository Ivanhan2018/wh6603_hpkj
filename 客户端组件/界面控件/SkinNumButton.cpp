...
BEGIN_MESSAGE_MAP(CSkinNumButton, CButton)
...
//绑定函数
VOID CSkinNumButton::PreSubclassWindow()
{
...
//时间消息
VOID CSkinNumButton::OnTimer(UINT_PTR nIDEvent)
{
	//过渡变量
...
//建立消息
INT CSkinNumButton::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//设置渲染
VOID CSkinNumButton::SetRenderImage(bool bRenderImage)
{
	//设置变量
...
//设置颜色
VOID CSkinNumButton::SetButtonColor(COLORREF crButtonText)
{
	//设置变量
...
//加载位图
bool CSkinNumButton::SetButtonImage(UINT uBitmapID, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置变量
	m_bRenderImage=bRenderImage;
...
//加载位图
bool CSkinNumButton::SetButtonImage(LPCTSTR pszResource, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置变量
	m_bRenderImage=bRenderImage;
	m_bTransparent=bTransparent;
...
//加载位图
bool CSkinNumButton::SetButtonImage(UINT uBitmapID, LPCTSTR pszSurface, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置变量
	m_bRenderImage=bRenderImage;
	m_bTransparent=bTransparent;
...
//加载位图
bool CSkinNumButton::SetButtonImage(LPCTSTR pszResource, LPCTSTR pszSurface, HINSTANCE hResInstance, bool bRenderImage, bool bTransparent)
{
	//设置资源
	m_pszSurface=pszSurface;
	m_pszResource=pszResource;
...
//调整位置
VOID CSkinNumButton::RectifyControl()
{
	//调整判断
...
//绘画文字
VOID CSkinNumButton::DrawButtonText(CDC * pDC, UINT uItemState)
{
	//获取文字
	CString strText;
...
//鼠标消息
VOID CSkinNumButton::OnMouseMove(UINT nFlags, CPoint Point)
{
...
//鼠标消息
LRESULT CSkinNumButton::OnMouseLeave(WPARAM wparam, LPARAM lparam)
{
	//设置变量
	m_bHovering=false;
...
//光标消息
BOOL CSkinNumButton::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	//设置光标
...
//销毁消息
VOID CSkinNumButton::OnNcDestroy()
{
	//状态变量
	m_bHovering=false;
	m_bRenderImage=true;
...
//绘画背景
BOOL CSkinNumButton::OnEraseBkgnd(CDC * pDC)
{
	return TRUE;
}

//绘画函数
VOID CSkinNumButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//获取位置
	CRect rcClient;
	GetClientRect(&rcClient);
	...
	if (ImageButton.IsNull()==false)
	{
		...
		if ((bDisable==false)&&(bButtonDown==false)&&(bTransitionDraw==true))
		{
			...
			bool bCreate = ImageBuffer.Create(rcClient.Width(),rcClient.Height(),32);
